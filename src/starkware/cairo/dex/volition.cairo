...
struct EncodingTables:
    member n_keys : felt
    member key_table_ptr : felt*
    member n_tokens : felt
    member token_table_ptr : felt*
end
...
func initialize_encoding_table(output_ptr : felt*) -> (
    ...
    return (output_ptr=output_ptr + 1 + table_size, table_ptr=output_ptr + 1, table_size=table_size)
end
...
func get_vault_change_encoding(
    ...
    let encoded_change = ((vault_index *
        BALANCE_BOUND + vault_state.balance) *
        ONCHAIN_DATA_KEY_INDEX_BOUND + key_index) *
        ONCHAIN_DATA_TOKEN_INDEX_BOUND + token_index
