...
struct L1VaultOutput:
    ...
    # minimal_initial_balance - the minimal balance the L1 vault must have prior to the execution of
    #   the batch, in order to prevent a temporary negative balance during the execution.
...
#   2. prev_value is the minimal initial balance needed to prevent a negative balance throughout the
#      execution of the batch .
#   3. new_value is the final balance after applying the batch, assuming the vault's initial balance
#      is the mentioned minimal initial balance.
#
# Hint arguments:
